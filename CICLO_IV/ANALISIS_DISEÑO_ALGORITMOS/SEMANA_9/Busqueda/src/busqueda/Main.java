...
public class Main {
    ...
    private static void ejecutarIndiceBinario(Scanner sc) throws IOException {
        ...
        if (idx.exists()) {
            boolean deleted = idx.delete();
        }

        // Crear índice desde cero
    ...
    private static void ejecutarListaInvertida(Scanner sc) throws IOException {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(DATOS_TXT))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                if (parts.length >= 3){
                    String ciudad = parts[2].trim().toLowerCase(); //
                    li.agregar(ciudad, idnum);// minúsculas
                    //li.agregar(parts[2], idnum);
                }
                idnum++;
            }
        }
        System.out.print("Buscar ciudad: ");
        //String ciudad = sc.nextLine().trim();
        String ciudad = sc.nextLine().trim().toLowerCase(); //también a minúsculas
